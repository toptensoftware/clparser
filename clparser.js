...
// Word break a line
function break_line(str, max_width)
{
    // Does it fit?
    ...
    for (let i=max_width-1; i>=0; i--)
    {
        if (str[i] == ' ')
        {
            return str.substring(0, i) + "\n" + break_line(str.substring(i+1), max_width);
        }
    }
...
function show_version(packageDir)
{
    // Load package
...
function show_help(spec, options)
{
    ...
    {
        show_version(options.packageDir);
        console.log("");
    }
    ...
    if (usagePrefix.length)
        usagePrefix += ' ';
    console.log(` Usage: ${usagePrefix}${format_usage(spec)}`);
    console.log("");
...
function parser(spec, options)
{
    ...
    // Return API
    return {
        parse: (args) => parse(args, spec),
        check: (cl) => check(spec, cl),
        format_usage: () => format_usage(spec),
        format_help: () => format_help(spec),
        show_version,
        show_help: () => show_help(options),
        handle_help: (cl) => handle_help(spec, cl, options),
    }
}
